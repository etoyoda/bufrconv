...
class BufrDecode
  ...
  def clear_operators
    @addwidth = @addscale = @adjscale = nil
    @addfield = @adjstrlen = @chgrefv = nil
    @super_refv = Hash.new
  end
...
=end
  ...
  def read_tape tb
    ...
    #
    ent = @tape[@pos]
    if refv = @super_refv[ent[:fxy]] then
      ent = ent.dup
      ent[:refv] = refv
    end
...
=end
  ...
  def run tb
    ...
    while desc = read_tape(tb)
      ...
      when :num, :code, :flags
        if @chgrefv then
          psdesc={:width=>@chgrefv, :refv=>0, :scale=>0}
          newrefv=@bufrmsg.readnum(psdesc)
          newrefv=-(newrefv ^ (2**(@chgrefv-1))) if newrefv & (2**(@chgrefv-1))
          @super_refv[desc[:fxy]] = newrefv
        else
...
=end
  ...
  def compile descs
    ...
    xd.size.times{|i|
      ...
      when /^203(\d\d\d)/ then
        result.push({ :type => :op03, :fxy => fxy, :yyy => $1.to_i })
      when /^204(\d\d\d)/ then

...
class BufrDecode
  ...
  def initialize tape, bufrmsg
    ...
    @cstack = []
    # operators
    @addwidth = @addscale = @addfield = @tmprefv = nil
    @ymdhack = ymdhack_ini
  end
    ...
    while desc = read_tape(tb)
      ...
      when :op03
        if desc[:yyy] == 255 then
          ...
          @tmprefv = @bufrmsg.readnum({:width=>desc[:yyy], :scale=>0, :refv=>0})
        end
        $stderr.puts "hack03: #{@bufrmsg.ahl} tmprefv=#{@tmprefv.inspect} yyy=#{desc[:yyy]})"
      when :op04
        if desc[:yyy].zero? then
          @addfield = nil
        else
